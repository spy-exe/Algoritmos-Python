...
def usePercent(usedSpace, totalSpace):
...
def main():
    ...
    usedSpaces = []
    for user in users:
        name, usedSpace = user.split()
        names.append(name)
        usedSpaces.append(int(usedSpace))

    totalSpace = sum(usedSpaces)
    mediumSpace = totalSpace / len(names)

    formatedInfos = []
    for i in range(len(names)):
        usedSpaceMB = bytesToMB(usedSpaces[i])
        usedPercent = usePercent(usedSpaces[i], totalSpace)
        formatedInfos.append(f"{i+1} {names[i]:<15} {usedSpaceMB:>10.2f} MB {usedPercent:>10.2f}%\n")
    ...
    with open("usuarios.txt", "w") as arquivo:
        ...
        arquivo.write("------------------------------------------------------------\n")
        arquivo.writelines(formatedInfos)
        arquivo.write("------------------------------------------------------------\n")
        arquivo.write(f"Espaço total ocupado: {bytesToMB(totalSpace):.2f} MB\n")
        arquivo.write(f"Espaço médio ocupado: {mediumSpace:.2f} MB\n")
